<template>
  <div class="team-match-list-container">
...
const loadMatches = async () => {
  ...
  try {
    const response = await api.get(`api/v4/teams/${teamId}/matches`)
...
const loadTeamName = async () => {
  const teamId = route.query.id
  try {
    const response = await api.get(`api/v4/teams/${teamId}`)
    teamName.value = response.data.shortName
  } catch (error) {
    console.error(error)
  }
}
